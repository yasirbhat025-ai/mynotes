...
import {
  ...
  Link2, Table as TableIcon, Upload, ChevronDown, ChevronUp
} from "lucide-react";
...
function TopBar({ db, view, setView, query, setQuery, saving, setMobileSidebar, notebook, subject, chapter }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 20px", borderBottom: `1px solid ${T.border}`, background: T.card, flexWrap: "wrap" }}>
          ...
          {notes.map((n) => (
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 600, fontSize: 14 }}>{n.title || "Untitled note"}</div>
...
function NoteEditorView({ db, persist, noteId, setView, chapter }) {
  ...
  return (
    <div style={{ maxWidth: 820, margin: "0 auto", padding: "20px 24px 80px" }}>
      <button className="focus-ring" onClick={() => setView({ level: "chapter", notebookId: db.subjects.find(s=>s.id===chapter?.subjectId)?.notebookId, subjectId: chapter?.subjectId, chapterId: chapter?.id })}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
        ...
        <button className="focus-ring" onClick={() => download(`${(note.title || "note").replace(/\s+/g, "_")}.txt`, stripHtml(note.html))}
          style={{ display: "flex", alignItems: "center", gap: 4, border: "none", background: "transparent", color: T.muted, cursor: "pointer", fontSize: 12 }}>
          <Download size={13} /> Export .txt
        </button>
      </div>

      <div className="scroll-thin" style={{ position: "sticky", top: 0, zIndex: 5, display: "flex", flexWrap: "wrap", gap: 3, padding: "8px 8px", background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, marginBottom: 14 }}>
        ...
        <Divider />
        <div style={{ position: "relative" }}>
          <ToolBtn onClick={() => setShowHiLite((s) => !s)} title="Highlight"><Highlighter size={14} /></ToolBtn>
          {showHiLite && (
      ...
      {findOpen && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12, padding: "6px 10px", borderRadius: 10, border: `1px solid ${T.border}`, background: T.card }}>
      ...
      {tableCtx && (
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 12, padding: "6px 10px", borderRadius: 10, border: `1px solid ${T.border}`, background: T.card, flexWrap: "wrap" }}>
